...
public class MjpegStream implements Runnable {
    ...
    private final static int SNAPSHOT_MAX_LENGTH = 1024*1024*2;
    private final static int SNAPSHOT_INTERVAL = 50; // 50 msec for 20fps
    ...
    private void renderSnapshotSequence()
    {
    	URI uri = URI.create(mUrl);
    	...
    	while (mRun)
    	{
    		...
    		if (contentLength > SNAPSHOT_MAX_LENGTH)
    			contentLength = SNAPSHOT_MAX_LENGTH;
    		if (contentLength <= 0)
    			continue;

    		byte[] data = new byte[contentLength];
    		try {
    			httpEntity.getContent().read(data);
    		...
    		if (onFrameReadCallback != null)
    			onFrameReadCallback.onFrameRead(bmp);
    		
    		long stampNow = System.currentTimeMillis();
    		int nextSleep = (int) (stampLast +SNAPSHOT_INTERVAL - stampNow);
    		stampLast = stampNow;
