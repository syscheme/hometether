// the c51 module covers pluse capturing and generating
...
void PCA_init()
{
	...
	CH = CL =0;
	CCAP0L = CCAP0H =0;
	CMOD = 0x09; // take the Fosc as the source clock of PCA
	CCAPM0 = 0x31; // capture at both up and down edges
...
void PCA_isr() interrupt 7 using 1
{
	if (CF)
	{
		// counter overflow, this must be an invalid pulse, put and END here
		*((uint8*)fifo_header++) = 0xff;
		*((uint8*)fifo_header++) = 0xff;
	}
	...
	{
		CCF0 =0;
		*((uint8*)fifo_header++) = CCAP0L;
		*((uint8*)fifo_header++) = CCAP0H | (PIN_PCA0 ? 0x80 :0x00);
	}
}
...
sbit InUse_any        = P0^1;	// the main flow sensor
sbit InUse_veranda    = P0^2;	// the flow sensor on the pipe to verandas
...
void main(void)
{
	...
	while (1)
	{
		...
		if (timerPump)
		{
			blinkReload =8; // every 1.6 second if pump run at inner loop
			if (byteStatus & FLAG_FullLoop)
				blinkReload = 4; // every 0.8 second if pump run at outer loop
		}

		if (timerValve2to1)
			blinkReload =2; // every 0.2 second if valve is executing
		...
		blinkReload =50; // blink every 5 seconds if at complete idle
