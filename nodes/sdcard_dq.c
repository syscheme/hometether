...
SD_Error SD_ReadBlock(u32 addr, u32 *readbuff, u16 BlockSize)
{
	...
	if (NULL == readbuff)
		return(errorstatus = SD_INVALID_PARAMETER);
	...
	if (SDIO_GetResponse(SDIO_RESP1) & SD_CARD_LOCKED)
		return(errorstatus = SD_LOCK_UNLOCK_FAILED);
...
SD_Error SD_WriteBlock(u32 addr, u32 *writebuff, u16 BlockSize)
{
	...
	if (DeviceMode == SD_POLLING_MODE)
	{
		...
		{
			SDIO_ClearFlag(SDIO_FLAG_DTIMEOUT);
			return(errorstatus = SD_DATA_TIMEOUT);
		}

		if (SDIO_GetFlagStatus(SDIO_FLAG_DCRCFAIL) != RESET)
		{
			SDIO_ClearFlag(SDIO_FLAG_DCRCFAIL);
			return(errorstatus = SD_DATA_CRC_FAIL);
		}

		if (SDIO_GetFlagStatus(SDIO_FLAG_TXUNDERR) != RESET)
		{
			SDIO_ClearFlag(SDIO_FLAG_TXUNDERR);
			return(errorstatus = SD_TX_UNDERRUN);
		}

		if (SDIO_GetFlagStatus(SDIO_FLAG_STBITERR) != RESET)
		{
			SDIO_ClearFlag(SDIO_FLAG_STBITERR);
			return(errorstatus = SD_START_BIT_ERR);
		}
	}
	else if (DeviceMode == SD_INTERRUPT_MODE)
	{
