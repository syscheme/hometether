...
void ISR_nRF_IR(void)  // Exti Line 5~9
{
	if (EXTI_GetITStatus(EXTI_Line6) != RESET)	// handling of IR receiver as EXTi
	{
		if (pendingPulseSeq.timeout >0)
			delayXusec(PLUSE_CAP_SAMPLE_USEC * PLUSE_MAX*2);
		else if (Pulses_capture(pluscapbuf, PLUSE_MAX*2+2, PLUSE_CAP_SAMPLE_USEC, irRecvBit))
		{
			pendingPulseSeq.timeout =0;
			pendingPulseSeq.ps.seqlen = sizeof(seq) -2;
			if (PulseSeq_compress(pluscapbuf, PLUSE_CAP_SAMPLE_USEC, &pendingPulseSeq.ps))
				pendingPulseSeq.timeout = PULSE_CapturedData_Timeout;
		}

//		Pulses_capture(&pulseCapBuf, irRecvBit);
		EXTI_ClearITPendingBit(EXTI_Line6);
	}

	if (EXTI_GetITStatus(EXTI_Line8) != RESET)	// handling of IRQ of nRF24L01
	{
//		ISBoard_setLed(1, 1);  // turn on LED
		processIRQ_nRF24L01(&nrf24l01);
//		ISBoard_setLed(1, 0);  // turn off LED
		EXTI_ClearITPendingBit(EXTI_Line8);
	}
//	ISBoard_setLed(1, 1); 
//	delayXmsec(1);
//	ISBoard_setLed(1, 0);// blink LED1
}

/*
void ISR_nRF_IR2(void)  // Exti Line 5~9
{
	int i=0, j=0, done=0, maxdur=0;
	if (EXTI_GetITStatus(EXTI_Line6) != RESET)	// handling of IR receiver as EXTi
	{
		if (pluse_intv_usec <=0)
			pluse_intv_usec =50;

		maxdur = 100*1000/pluse_intv_usec;

		for (i=0; !done && i <PLUSE_MAX; i++)
		{
			j = i*2;
			for (plusseq[j] =1; !done && 0 ==irRecvBit();) 
			{
				if (plusseq[j]++ > maxdur)
				{ done=TRUE; break; }

				delayXusec(pluse_intv_usec);
			}

			for (plusseq[j+1] = plusseq[j], j++; !done && 1 ==irRecvBit();) 
			{
				if (plusseq[j]++ > maxdur)
				{ done=TRUE; break; }
		...
		plusseq[++j] =0xffff; plusseq[++j] =0xffff;

		if (j>10)
			pulseseq_timeout = PULSE_SIG_TIMEOUT;
...
void  BSP_IntInit (void)
{
	...
	BSP_IntVectSet(BSP_INT_ID_EXTI9_5,        ISR_nRF_IR);
	BSP_IntVectSet(BSP_INT_ID_DMA1_CH1,       ISR_ADC1DMA);
#ifdef USB_ENABLED
	BSP_IntVectSet(BSP_INT_ID_USB_LP_CAN_RX0, ISR_USBRX0);
#endif // USB_ENABLED
}
