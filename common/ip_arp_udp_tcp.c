...
#define MAX_UDP_PAYLOAD (1400 - UDP_HEADER_LEN -IP_HEADER_LEN -ETH_HEADER_LEN -10)
...
void init_ip_arp_udp_tcp(const uint8_t *mymac, const uint8_t *myip, uint16_t wwwp)
{
	uint8_t i=0;
	wwwport = wwwp;
	for (i=0; i<4; i++)
...
uint8_t eth_type_is_ip(uint8_t *packet, uint16_t len)
{
...
uint8_t is_to_myip(uint8_t *packet, uint16_t len)
{
	return match_dest_ip(ipaddr, packet, len);
}

uint8_t match_dest_ip(const uint8_t *destIp, uint8_t *packet, uint16_t len)
{
	uint8_t i=0;

	for (i=0; i<4; i++)
	{
		if (packet[IP_DST_P+i]!=destIp[i])
			return(0);
	}
...
uint8_t match_src_ip(const uint8_t *sourceIp, uint8_t *packet, uint16_t len)
{
	uint8_t i=0;

	for (i=0; i<4; i++)
	{
		if (packet[IP_SRC_P+i]!=sourceIp[i])
			return(0);
	}
...
void make_ip(uint8_t *packet, const uint8_t* destIp)
{
	uint8_t i=0;
	if (NULL == destIp)
...
void make_tcphead(uint8_t *packet,uint16_t rel_ack_num,uint8_t mss,uint8_t cp_seq)
{
	...
	packet[TCP_SRC_PORT_L_P]=wwwport &0xff;
	packet[TCP_SRC_PORT_H_P]=wwwport >>8;
...
void echo_reply_from_request(void* nic, uint8_t *packet,uint16_t len)
{
	...
	//
	nic_send_packet(nic, packet, len);
}
...
void udp_reply_from_request(void* nic, uint8_t *packet, char *data, uint8_t datalen, uint16_t port)
{
	...
	nic_send_packet(nic, packet, UDP_HEADER_LEN +IP_HEADER_LEN +ETH_HEADER_LEN +datalen);
}
...
void tcp_synack_from_syn(void* nic, uint8_t *packet)
{
	...
	nic_send_packet(nic, packet, IP_HEADER_LEN+TCP_HEADER_LEN_PLAIN+4+ETH_HEADER_LEN);
}
...
uint16_t fill_tcp_data_str(uint8_t *packet, uint16_t pos, const char* progmem_s)
{
	char c;
...
// tcp data. Returns the position at which the string after
// this string could be filled.
uint16_t fill_tcp_data_p(uint8_t *packet, uint16_t pos, const uint8_t* s, uint16_t len)
{
	// fill in tcp data at position pos
	//
	// with no options the data starts after the checksum + 2 more bytes (urgent ptr)
...
void tcp_ack_from_any(void* nic, uint8_t *packet)
{
	...
	nic_send_packet(nic, packet, IP_HEADER_LEN +TCP_HEADER_LEN_PLAIN +ETH_HEADER_LEN);
}
...
void tcp_ack_with_data(void* nic, uint8_t *packet,uint16_t dlen)
{
	...
	nic_send_packet(nic, packet, IP_HEADER_LEN +TCP_HEADER_LEN_PLAIN +dlen +ETH_HEADER_LEN);
}

uint16_t fill_udp_data_p(uint8_t *packet, uint16_t pos, const uint8_t *data, uint16_t datalen)
{
	int16_t i=datalen;

	if (i + pos >MAX_UDP_PAYLOAD)
		i = i - pos;
	...
	for (datalen =i, i=0; i<datalen; i++)
		packet[UDP_DATA_P+pos+i]=data[i];
...
//	groupIp[0] |= 0xe0;	groupIp[0] &= 0xef;
	...
	packet[IP_TTL_P] = 2; // should be 1 to only limited in the LAN

	packet[ETH_TYPE_H_P] =ETHTYPE_IP_H_V;
	packet[ETH_TYPE_L_P] =ETHTYPE_IP_L_V;
	make_eth(packet, destMac);

	if (datalen>MAX_UDP_PAYLOAD)
		datalen=MAX_UDP_PAYLOAD;
